...
class AndroidPdfGenerator(
    ...
    override suspend fun generateInvoicePdf(invoice: Invoice, profile: BusinessProfile): String {
        return withContext(Dispatchers.IO) {
            ...
            // 1. Draw Address Line 1
            canvas.drawText(profile.addressLine1, 40f, yPosition, paint)
            yPosition += 15f

            // 2. Draw Address Line 2
            if (profile.addressLine2.isNotBlank()) {
                canvas.drawText(profile.addressLine2, 40f, yPosition, paint)
                yPosition += 15f
            }
            canvas.drawText(
                "${profile.state.stateName} - ${profile.pincode}",
                40f,
                yPosition,
                paint
            )
